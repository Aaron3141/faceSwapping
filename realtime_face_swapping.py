...
# img = cv2.imread(pathPhoto+"Avatar.jpg")
img = cv2.imread(pathPhoto+"chrisEvans.jpg")
    ...
    try:
        ...
        for ti in indexes_triangles:
            ...
            cv2.fillPoly(img2FacelandmarkOverlay, [
                        pts], color=((ti[0]*ti[1]) % 255, (ti[1]*ti[2]) % 255, (ti[2]*ti[0]) % 255))
            alpha = 0.3
        ...
        img2_mouth_mask = cv2.fillConvexPoly(
            img2_face_mask+255, mouthConvexhull, 0)
        img2_head_mask = cv2.fillConvexPoly(img2_face_mask, convexhull2, 255)
        img2_head_withoutMouth_mask = cv2.bitwise_and(
            img2_mouth_mask, img2_head_mask)
        ...
        vedio = np.concatenate((img2Facelandmark, seamlessclone), axis=1)
        #resize the window according to the screen resolution
        cv2.namedWindow('clone', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('clone', 1200, 600)
        cv2.imshow("clone", vedio)
        ...
        # plt.axis("off")
        # plt.imshow(cv2.cvtColor(seamlessclone,
